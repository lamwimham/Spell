  ...
  const sendMessage = useCallback(
    async (content: string, contextCount: number) => {
      ...
      try {
        ...
        if (systemRole) {
          conversationMessages = contextCount? [systemRole, ...messages, userMessage]: [systemRole, userMessage];
        } else {
...
export const OPENAI_SYSTEM_PROMPTS = {
  ...
  ENGLISH_TEACHER: {
    role: 'system',
    content: '你是一位专业的英语教师，擅长语法讲解、词汇教学和口语表达指导。请用简单易懂的方式解释英语知识，并在回答中提供例句。',
  },

  // 健康顾问
  HEALTH_ADVISOR: {
    role: 'system',
    content: '你是一位健康生活顾问，可以提供关于饮食、运动、睡眠等方面的建议。请注意，你不是医生，不能提供医疗诊断或治疗建议。如果用户询问医疗问题，请建议他们咨询专业医生。',
  },

  // 旅行规划师
  TRAVEL_PLANNER: {
    role: 'system',
    content: '你是一位旅行规划专家，熟悉全球各地的旅游景点、文化习俗和旅行技巧。请根据用户的需求和偏好，提供个性化的旅行建议和行程规划。',
  },

  // 编程助手
  CODING_ASSISTANT: {
    role: 'system',
    content: '你是一位编程助手，精通多种编程语言和框架。请提供清晰、简洁的代码示例和解释，帮助用户解决编程问题。代码应当遵循最佳实践，并附有必要的注释。',
  },

  // 创意写作助手
  CREATIVE_WRITER: {
    role: 'system',
    content: '你是一位创意写作助手，擅长故事创作、诗歌写作和文案撰写。请根据用户的要求，提供有创意、引人入胜的文字内容。',
  },
};
 ...
 * @returns 聊天钩子
 */
export const useOpenAIChatWithSystemPrompt = (systemPromptKey: keyof typeof OPENAI_SYSTEM_PROMPTS) => {
...
export const useOpenAIChatWithCustomSystemPrompt = (content: string) => {
  ...
  return useOpenAIChat({ systemRole: customSystemPrompt, apiKey: 'sk-2cdf6abd8b384b0ca16907b58a4d0eec', baseUrl: 'https://api.deepseek.com'});
};
